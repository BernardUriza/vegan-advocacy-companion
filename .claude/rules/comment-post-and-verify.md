...
| `comment-prepare` falla con `"target article not found"` (stage `open`) | el `--anchor` no matchea el VERBATIM del target — FB tiene **typos** del autor (Shane escribió "natraul", no "natural") | elegir el anchor de una frase del **texto scrapeado real** (de `thread-extract`), evitando la palabra que pueda venir mal escrita. Un fragmento sin la palabra dudosa ("subspecies of the term") matchea donde "as natural as eating plants" falla |
| Tras un `comment-prepare` que falló en `open`, queda una **tab de FB huérfana** | el script abrió la tab persistente antes de fallar al localizar el target | identificar la tab viva por **contenido del composer** (no por índice), no por la posición en `list_pages`; cerrar las huérfanas al cerrar el lote |
