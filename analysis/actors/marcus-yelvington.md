# Marcus Yelvington

> GENERATED from `data/actors.json` by `scripts/gen-dossiers.mjs`. Do not hand-edit — edit the JSON and regenerate.

- **user_id:** 100094637395621
- **Bando:** anti-vegan
- **Veredicto:** persuadible · **Registro:** compasivo
- **Postura núcleo:** Concede que el abuso animal esta mal; reencuadra el desacuerdo a que-hacer-al-respecto (policy), no al estatus de propiedad.

## Análisis

Actor nuevo (post 27330), buena fe. Abre concediendo el common ground y desplazando a politica. Persuadible: el reframe es honesto, no hostil. El oro vive en la buena fe (moat).

## Tácticas

- **Concede-and-Reframe-to-Policy** (`concesion_reframe_policy`) — Concede el punto moral obvio (ej. "animal abuse is wrong") y desplaza el desacuerdo a la POLITICA ("where we differ is what should be done about it"), esquivando la pregunta del ESTATUS que hace posible el abuso. Suele ser de buena fe, pero mueve el debate del fundamento a la accion.
  - _Contra:_ Aceptar el common ground de frente y mostrar que el desacuerdo es ANTERIOR a la politica: si ya llamamos abuso a maltratar al ser, es porque hay alguien ahi; el estatus de propiedad es el SUELO del abuso, no un tema aparte. Devolver al hueso: que justifica tener a un sujeto consciente como propiedad de entrada.

## Counter-arsenal

> Munición candidata por táctica (`getFrameworksByTactic`). Surfaceo, NO la jugada — etapa-3 elige UN solo framework respetando su `attack_surface`.

- **Concede-and-Reframe-to-Policy** (`concesion_reframe_policy`)
  - `algo-a-alguien-sujeto-derecho` → _deploy as:_ marco

## Qué NO hacer

No acusarlo; no aceptar el reencuadre a policy como el desacuerdo real; no eje bienestarista; no abrir con su nombre.

## Log de interacciones

### Hilo `27330688236588406` — 2026-07-02 · outcome: **silent**
- **Su jugada:** Concede que el abuso animal esta mal; el desacuerdo es que-hacer-al-respecto (if anything, should be done about it) — reframe a policy.
- **Nuestra respuesta:** Concedi el common ground (si, ambos vemos mal el abuso) y mostre que el desacuerdo es ANTERIOR a la politica: si ya llamamos abuso a maltratar al animal es porque hay alguien ahi; el estatus de propiedad es el suelo del abuso, no un tema aparte (no hablamos de abuse hacia una roca/tractor). Cierre: que justifica tener a un sujeto consciente como propiedad de entrada.

_Hilos: 27330688236588406_
