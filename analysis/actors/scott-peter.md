# Scott Peter

> GENERATED from `data/actors.json` by `scripts/gen-dossiers.mjs`. Do not hand-edit — edit the JSON and regenerate.

- **user_id:** 100004662660380
- **Perfil:** https://www.facebook.com/groups/2295597740524135/user/100004662660380/
- **Bando:** ambiguo
- **Veredicto:** audiencia · **Registro:** compasivo
- **Postura núcleo:** No argumenta él mismo; afirma que otros ya respondieron bien y que el problema es que Bernard no se explica. Veneer de buena fe ('me gusta que preguntes') sobre un dismissal.

## Análisis

Civil, abre reconociendo la pregunta — pero el cuerpo es un esquive: apela a un head-count de refutaciones ('al menos 3 personas dieron buenas respuestas') sin nombrar ninguna, y tone-policea ('explícate mejor'), trasladando la carga a Bernard. Las 'correcciones' que invoca (Tiana=cognición, Nate=jerarquía/poder, Michael='no es un someone') respondieron can/biología/estatus-negado, NO la pregunta del post (qué justifica poseer a un sujeto). El tono civil lo hace candidato a audiencia/persuadible: no es troll. Educativo para el lurker desactivar el 'ya te respondieron' mostrando que respondieron OTRA pregunta.

## Tácticas

- **Appeal to Prior Refutation / You've-Been-Answered Deflection** (`apelacion_refutacion_previa`) — Claims the questioner has already been refuted by unnamed others ('3 people gave good answers') and reframes the still-open question as the questioner's failure to communicate ('you should explain yourself better'). Makes no argument itself: outsources the refutation and shifts the burden onto the asker.
  - _Contra:_ Don't accept the premise that the prior answers hit the actual question. Name which question they answered (usually 'can we / are they smart enough / are they really a someone') versus the one asked (what justifies owning a subject that wants to live). The question staying open isn't a communication failure — it's the answer never arriving. Return the burden.
- **Burden of Proof Reversal** (`inversion_carga`) — Flips the burden of proof so the vegan must justify veganism rather than the carnist justifying exploitation. 'The onus is not on me, who is normal — it's on the fringe ideology.'
  - _Contra:_ Name it and return the burden: 'The burden of proof is on whoever makes a claim. You're claiming it's justified to impose suffering on a sentient being for something we don't need. That's the claim that needs a justification, not the default of avoiding unnecessary harm.'

## Counter-arsenal

> Munición candidata por táctica (`getFrameworksByTactic`). Surfaceo, NO la jugada — etapa-3 elige UN solo framework respetando su `attack_surface`.

- **Appeal to Prior Refutation / You've-Been-Answered Deflection** (`apelacion_refutacion_previa`)
  - `algo-a-alguien-sujeto-derecho` → _deploy as:_ marco
- **Burden of Proof Reversal** (`inversion_carga`)
  - `esclavitud-paralelismo` → _deploy as:_ marco
  - `crueldad-no-es-la-causa` → _deploy as:_ marco
  - `consideracion-moral-no-cuanto-sufre` → _deploy as:_ premisa_portante
  - `resistencia-al-cambio-es-defensa-del-privilegio` → _deploy as:_ auto-disciplina-del-activista

## Qué NO hacer

NO re-explicar bajo demanda ni defenderte de 'no te explicas' (acepta su premisa). NO perseguir cuáles 3 personas. Reconocer la buena fe del tono, nombrar que las correcciones respondieron can/biología/estatus-negado y no la pregunta del eje, devolver la carga con alguien-vs-algo como PREGUNTA (no premisa probada — su attack_surface). Eje propiedad/sujeto poseído, no daño.

## Log de interacciones

### Hilo `27330688236588406` — 2026-06-27 · outcome: **pending**
- **Su jugada:** Dismissal por consenso + tone-policing: 'al menos 3 personas ya te dieron buenas respuestas y sigues diciendo que no entienden — explícate mejor'. No argumenta él; apela a un head-count de refutaciones (Tiana/Nate/Michael) y traslada la carga.
- **Nuestra respuesta:** Reply etiquetada, registro compasivo-firme. Reconoce que no insultó. Nombra que las 3 'correcciones' son criterios distintos (cognición/jerarquía/'no es someone') para la MISMA pregunta: cuándo un individuo vivo deja de tratarse como un quién y pasa a ser un qué poseíble. Eso ES la pregunta, no la respuesta. 'Explícate mejor' no aplica: la pregunta sigue abierta porque nadie defendió POR QUÉ su línea justifica el cambio de estatus, solo lo asume. Carga devuelta. Framework alguien-vs-algo como PREGUNTA (no premisa) — el coagent afiló mi versión que presuponía el 'quién'.

_Hilos: 27330688236588406_
